...
class NewRecipes extends Component {
    ...
    handleClick(event){  // It adds the clicked recipe into state and prevents duplicate entries
        ...
        let id = event.target.id.split(',')  //"id" is an array of two strings.  id[0] is equal to the index of the button.  id[1] is equal to the recipe uri for the corresponding button.
        ...
        let recipeObj = recipes.filter(resp => resp.recipe.uri === id[1])[0]  //recipeObj is the first element of a 1 element array of a sincle recipe.  recipeObj = [{...}][0] = {...}
        ...
        if (clicked[index]){
          ...
          this.setState({ clicked })
              ...
              deleteRecipe(recipeObj.recipe.url)
        }
      ...
      console.log("This is the clicked array from state: ", this.state.clicked);
        return(
          ...
          <Panel className="flex-item" defaultExpanded>
               ...
               <ButtonFunction id={`${index},${element.recipe.uri}`} onClick= {this.handleClick.bind(this)} style={!this.state.clicked[index] ? "danger" : "warning"} text={!this.state.clicked[index] ? "Save Recipe" : "Unsave Recipe"}/>
                    <h3>
